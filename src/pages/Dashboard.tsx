...
const Dashboard = () => {
  ...
  const canSeeFinance = hasAccess('finance');
  ...
  return (
    <div className="animate-slide-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {kpis.map((kpi, index) => {
          // Hide financial KPI for gestionnaire
          if (index === 0 && !canSeeFinance) {
            return (
              <div key={index} className="stat-card opacity-50">
                <div className="flex items-center gap-3 mb-3">
                  <div className="p-3 rounded-2xl bg-muted">
                    <kpi.icon className="text-muted-foreground" size={22} />
                  </div>
                </div>
                <p className="text-sm text-muted-foreground mb-1">{kpi.label}</p>
                <p className="text-lg font-bold text-muted-foreground">Accès restreint</p>
              </div>
            );
          }
          return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        {canSeeFinance ? (
          ...
          <div className="card-xl p-6 flex items-center justify-center">
            <div className="text-center text-muted-foreground">
              <TrendingUp size={40} className="mx-auto mb-2 opacity-30" />
              <p className="font-medium">Accès restreint</p>
              <p className="text-sm">Les données financières ne sont pas accessibles pour votre rôle.</p>
            </div>
          </div>
        )}
